...
$(document).ready(function () {
    ...
    $generateBtn.on("click", function () {
        ...
        $("#result_input").val(gp);
        for (var carac in checkPass(gp)) {
            draw.forEach(c => {
                if (c.carac == carac) {
                    c.assets.forEach(asset => {
                        asset.svg.animate({ d: asset.d[checkPass(gp)[carac]] }, 250, mina.easeinout);
                    });
                }
            });
        }
        ...
        for (var carac in checkPass(gp)) {
            $('.pass-wrapper [data-carac="' + carac + '"]').attr('data-level', checkPass(gp)[carac]);
        }
...
const config = {
    ...
    fillColor: '#f7c926',
    shadow: s.filter(Snap.filter.shadow(0, 0, 2, '#000000', 0.3)),
    strengthSteps: {
        length: [0, 6, 10, 16],
        specials: [0, 2, 4, 6],
        numbers: [0, 2, 4, 6],
